...
// dist/icon-layer/icon-layer.wgsl.js
var shaderWGSL = (
  /* wgsl */
  `struct IconUniforms {
  sizeScale: f32,
...
var IconLayer = class extends import_core6.Layer {
  getShaders() {
    return super.getShaders({ vs: icon_layer_vertex_glsl_default, fs: icon_layer_fragment_glsl_default, source: shaderWGSL, modules: [import_core6.project32, import_core6.color, import_core6.picking, iconUniforms] });
  }
  initializeState() {
    this.state = {
...
var LineLayer = class extends import_core7.Layer {
  ...
  getShaders() {
    return super.getShaders({ vs: line_layer_vertex_glsl_default, fs: line_layer_fragment_glsl_default, source: shaderWGSL2, modules: [import_core7.project32, import_core7.color, import_core7.picking, lineUniforms] });
  }
  // This layer has its own wrapLongitude logic
  get wrapLongitude() {
    return false;
...
// dist/point-cloud-layer/point-cloud-layer.wgsl.js
...
var PointCloudLayer = class extends import_core8.Layer {
  getShaders() {
    return super.getShaders({
      ...
      modules: [import_core8.project32, import_core8.color, import_core9.gouraudMaterial, import_core8.picking, pointCloudUniforms]
    });
  }
  initializeState() {
...
// dist/scatterplot-layer/scatterplot-layer.wgsl.js
...
var ScatterplotLayer = class extends import_core10.Layer {
  getShaders() {
    return super.getShaders({
      vs: scatterplot_layer_vertex_glsl_default,
      fs: scatterplot_layer_fragment_glsl_default,
      source: scatterplot_layer_wgsl_default,
      modules: [import_core10.project32, import_core10.color, import_core10.picking, scatterplotUniforms]
    });
  }
  initializeState() {
    this.getAttributeManager().addInstanced({
  ...
  draw({ uniforms }) {
    ...
    const scatterplotProps = {
      ...
      billboard,
      antialiasing,
      radiusUnits: import_core10.UNIT[radiusUnits],
      radiusScale,
      radiusMinPixels,
      radiusMaxPixels,
      lineWidthUnits: import_core10.UNIT[lineWidthUnits],
      lineWidthScale,
      lineWidthMinPixels,
      lineWidthMaxPixels
    };
...
function tesselateColumn(props) {
  ...
  let { vertices } = props;
  if (vertices) {
    import_core11.log.assert(vertices.length >= nradial);
...
var ColumnLayer = class extends import_core12.Layer {
  getShaders() {
    const defines = {};
    ...
    return super.getShaders({
      ...
      defines,
      modules: [import_core12.project32, flatShading ? import_core12.phongMaterial : import_core12.gouraudMaterial, import_core12.picking, columnUniforms]
    });
  }
  /**
  ...
  draw({ uniforms }) {
    ...
    const columnProps = {
      ...
      widthUnits: import_core12.UNIT[lineWidthUnits],
...
var END_CAP = 2;
var INVALID = 4;
var PathTesselator = class extends import_core14.Tesselator {
  constructor(opts) {
    super({
      ...opts,
      attributes: {
...
var PathLayer = class extends import_core15.Layer {
  getShaders() {
    return super.getShaders({ vs: path_layer_vertex_glsl_default, fs: path_layer_fragment_glsl_default, modules: [import_core15.project32, import_core15.picking, pathUniforms] });
  }
  get wrapLongitude() {
    return false;
  }
  ...
  draw({ uniforms }) {
    ...
    const pathProps = {
      ...
      billboard,
      widthUnits: import_core15.UNIT[widthUnits],
      widthScale,
      miterLimit,
      widthMinPixels,
      widthMaxPixels
...
var PolygonTesselator = class extends import_core16.Tesselator {
  constructor(opts) {
...
var SolidPolygonLayer = class extends import_core17.Layer {
  getShaders(type) {
    return super.getShaders({
      ...
      modules: [import_core17.project32, import_core17.gouraudMaterial, import_core17.picking, solidPolygonUniforms]
    });
  }
  get wrapLongitude() {
    return false;
...
var PolygonLayer = class extends import_core18.CompositeLayer {
  initializeState() {
    this.state = {
      paths: [],
      pathsDiff: null
    ...
    if (this.props.getLineDashArray) {
      import_core18.log.removed("getLineDashArray", "PathStyleExtension")();
    }
  }
  ...
  _getPaths(dataRange = {}) {
    ...
    const { iterable, objectInfo } = (0, import_core18.createIterable)(data, startRow, endRow);
    for (const object of iterable) {
      objectInfo.index++;
...
// dist/text-layer/text-uniforms.js
var CONTENT_ALIGN = {
  none: 0,
  start: 1,
  center: 2,
...
// dist/text-layer/multi-icon-layer/multi-icon-layer-vertex.glsl.js
...
var MultiIconLayer = class extends icon_layer_default {
  ...
  updateState(params) {
    ...
    if (!props.sdf && props.outlineWidth) {
      import_core19.log.warn(`${this.id}: fontSettings.sdf is required to render outline`)();
    }
  }
  draw(params) {
...
function buildMapping2({ characterSet, measureText: measureText2, buffer, maxCanvasWidth, mapping = {}, xOffset = 0, yOffsetMin = 0, yOffsetMax = 0 }) {
  const row = 0;
  let x = xOffset;
  let yMin = yOffsetMin;
  let yMax = yOffsetMax;
...
function transformRow(line, startIndex, endIndex, iconMapping, leftOffsets, rowSize) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if (frame) {
      ...
      import_core20.log.warn(`Missing character: ${character} (${character.codePointAt(0)})`)();
      leftOffsets[i] = x;
...
function setFontAtlasCacheLimit(limit) {
  import_core21.log.assert(Number.isFinite(limit) && limit >= CACHE_LIMIT, "Invalid cache limit");
...
var TextBackgroundLayer = class extends import_core22.Layer {
  getShaders() {
    return super.getShaders({
      vs: text_background_layer_vertex_glsl_default,
      fs: text_background_layer_fragment_glsl_default,
      modules: [import_core22.project32, import_core22.picking, textBackgroundUniforms, textUniforms]
    });
  }
  initializeState() {
    this.getAttributeManager().addInstanced({
  ...
  draw({ uniforms }) {
    ...
    const textBackgroundProps = {
      ...
      borderRadius,
      padding,
      sizeUnits: import_core22.UNIT[sizeUnits],
      sizeScale,
      sizeMinPixels,
      sizeMaxPixels
    };
...
var TextLayer = class extends import_core23.CompositeLayer {
  constructor() {
    super(...arguments);
  ...
  initializeState() {
    ...
    if (this.props.maxWidth > 0) {
      import_core23.log.once(1, "v8.9 breaking change: TextLayer maxWidth is now relative to text size")();
    }
  }
  ...
  _updateText() {
    ...
    if (textBuffer && startIndices) {
      ...
      const { iterable, objectInfo } = (0, import_core23.createIterable)(data);
      startIndices = [0];
      numInstances = 0;
      for (const object of iterable) {
        objectInfo.index++;
...
function getGeojsonFeatures(geojson) {
  ...
  import_core24.log.assert(geojson.type, "GeoJSON does not have type");
  switch (geojson.type) {
    case "Feature":
      return [geojson];
    case "FeatureCollection":
      import_core24.log.assert(Array.isArray(geojson.features), "GeoJSON does not have features array");
...
function separateGeojsonFeatures(features, wrapFeature, dataRange = {}) {
  ...
  for (let featureIndex = startRow; featureIndex < endRow; featureIndex++) {
    ...
    if (geometry.type === "GeometryCollection") {
      import_core24.log.assert(Array.isArray(geometry.geometries), "GeoJSON does not have geometries array");
...
function separateGeometry(geometry, separated, wrapFeature, sourceFeature, sourceFeatureIndex) {
  ...
  if (!validateGeometry(type, coordinates)) {
    import_core24.log.warn(`${type} coordinates are malformed`)();
    return;
  }
  switch (type) {
    case "Point":
...
function validateGeometry(type, coordinates) {
  ...
  import_core24.log.assert(nestLevel, `Unknown GeoJSON type ${type}`);
...
var GeoJsonLayer = class extends import_core25.CompositeLayer {
  initializeState() {
    this.state = {
      layerProps: {},
      features: {},
